...
- `prestado_a` (`str`): **Inicialmente será una cadena** que almacenará el nombre del usuario que lo tiene. Valor inicial: `None` o `"Sin prestar"`. _(Nota para el alumno: en la siguiente fase este atributo cambiará para almacenar un objeto, pero por ahora trabaja con strings)_.
...
_(Utiliza el módulo `datetime` para obtener la marca temporal y abre el archivo en modo append `"a"` con codificación `utf-8`)._
...
_(El archivo `biblioteca_log.txt` debe haberse actualizado automáticamente tras cada llamada válida a `prestar` o `devolver`)._
...
4. **El reto de la lógica:** Modificar el préstamo para que las revistas se presten por 7 días y los DVDs por 2 días (los libros se quedan en 3). _Hay dos maneras de hacerlo, pasar los días al `__init__` de Recurso o sobrescribir el método `prestar` usando `super()`_
...
Añadir un método `generar_informe_general()` que use `yield` para ir devolviendo uno a uno los strings con la información de _todos_ los recursos de la biblioteca. (Esto evita cargar una lista gigante en memoria si la biblioteca fuera enorme).
